...
int main(int argc, char **argv) {
	 ...
	 *******************/
	char role = *(argv[3]);
	char handshake[17] = {'H', 15, 'g',
					   'n', 'e', 'd', 'p', 'e', 'n',
					   'k', 'e', 'c', 'h', 'e', 'z', 'o', 'n'};
	std::map<char, char> movementMap{{'w', 'N'}, {'a', 'W'}, {'s', 'S'}, {'d', 'E'}, {'.', '.'}};

	client.connection.send(handshake);
	while (true) {
		char act = (char)std::getchar();

		if (role == 'g') {
			if (movementMap.contains(act)) {
				char moveMessage[3] = {'M', 1, movementMap[act]};
				client.connection.send(moveMessage);
			}
		}
		...
		client.poll([](Connection *c, Connection::Event event){
			if (event == Connection::OnOpen) {
				...
				for (int i = 0; i < c->recv_buffer.size(); i++) {
					if (i != 1) printf("%c", c->recv_buffer[i]);
					else printf("%u", c->recv_buffer[i]);
				}
				printf("\n");
				// try {
				// 	do {
